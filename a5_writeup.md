...
I learned about what a stack was and how to use it. I've never used stacks before so it was a new and hard to grasp concept at first so I found myself constantly rereading the slides to overcome it. This made the BFS and DFS a challenge for me as i had to understand the stack class. 
...
Things like scheduling use BFS and DFS as well as constraint satisfaction. backtracking when somthing doesn't work out is a crucial feature for many programs and algorithms. 
...
LIFO says that the last element added is the first one to be removed. Fifo says that the element first added would be the first to be removed. DFS is better with stack because it allows for deep searching by going down a path. BFS is good with queue because it allows for the level by level exploration of BFS.